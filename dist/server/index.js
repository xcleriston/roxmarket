    ...
    async function refresh() {
      try {
        ...
        const tradesHtml = trades.map(t => \`
          <tr onclick="window.open('https://polymarket.com/event/\${t.eventSlug || t.slug}', '_blank')" style="cursor: pointer">
            <td style="font-size: 0.75rem; color: var(--text-dim)">\${new Date(t.timestamp).toLocaleTimeString()}</td>
            <td style="color: var(--accent); font-weight: 700">\${t.displayTrader}</td>
            <td style="font-size: 0.85rem; font-weight: 600; max-width: 250px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis" title="\${t.title}">
                \${t.title || t.slug || '---'}
            </td>
            <td><span style="color: \${t.side === 'BUY' ? 'var(--success)' : 'var(--danger)'}; font-weight: 700">\${t.side}</span></td>
            <td style="font-weight: 700">$\${(t.usdcSize || 0).toFixed(2)}</td>
            <td>
                <span class="badge" style="background: rgba(59, 130, 246, 0.1); color: var(--accent); border: 1px solid rgba(59, 130, 246, 0.2)">
                    \${t.processedBy?.length || 0} Copiaram
                </span>
            </td>
            <td>\${t.bot ? '<span class="badge badge-ready">✓ EXECUTADO</span>' : '<span class="badge badge-setup">⚡ DETECTADO</span>'}</td>
          </tr>
        \`).join('');
