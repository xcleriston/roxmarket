...
const recordStatus = async (activityId, followerId, status, details, extra, traderAddress) => {
    try {
        ...
        await ActivityModel.updateOne({ _id: activityId }, { $set: { [`followerStatuses.${followerId}`]: { status, details, timestamp: new Date(), ...extra } } });
    }
    catch (e) {
        Logger.error(`Failed to record status for ${followerId}: ${e}`);
    }
};
const postOrder = async (clobClient, condition, my_position, user_position, trade, my_balance, followerId, userConfig, my_positions = [], proxyAddress, traderAddress) => {
        ...
        while (remaining > 0.05 && retry < retryLimit) {
            try {
                ...
                if (resp.success) {
                    ...
                    retry++;
                    if (retry >= retryLimit)
                        await recordStatus(trade._id, followerId, 'ERRO (API)', err, undefined, traderAddress);
                }
            }
            catch (e) {
