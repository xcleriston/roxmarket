...
const recordStatus = async (activityId, followerId, status, details, extra) => {
    try {
        console.log(`[RECORD_STATUS] ${followerId} -> ${status}: ${details || ''} ${extra ? JSON.stringify(extra) : ''}`);
        await Activity.updateOne({ _id: activityId }, { $set: { [`followerStatuses.${followerId}`]: { status, details, timestamp: new Date(), ...extra } } });
    }
    catch (e) {
        Logger.error(`Failed to record status for ${followerId}: ${e}`);
    }
};
const postOrder = async (clobClient, condition, my_position, user_position, trade, my_balance, followerId, userConfig, my_positions = [], proxyAddress) => {
    const isMirror100 = userConfig.mode === 'MIRROR_100';
    const config = {
        strategy: isMirror100 ? CopyStrategy.PERCENTAGE : (userConfig.strategy || CopyStrategy.PERCENTAGE),
    ...
    if (effectiveCondition === 'buy') {
        ...
        while (remaining > 0.05 && retry < retryLimit) {
            try {
                ...
                const orderSizeUSD = useTradePrice ? remaining : Math.min(remaining, parseFloat(String(bestAsk.size)) * parseFloat(String(bestAsk.price)));
                const orderTokens = orderSizeUSD / parseFloat(String(bestAsk.price));
                const isLimit = trade.orderType === 'LIMIT' || orderSizeUSD < 1.0;
                let resp;
                if (isLimit) {
                    resp = await clobClient.createAndPostOrder({
                        tokenID: trade.asset,
                        price: useTradePrice ? (orderPrice || parseFloat(String(bestAsk.price))) : parseFloat(String(bestAsk.price)),
                        side: Side.BUY,
                        size: orderTokens,
                    }, { tickSize: "0.01" });
                }
        ...
        while (remaining > 0.05 && retry < retryLimit) {
            try {
                ...
                const sellSize = Math.min(remaining, parseFloat(String(bestBid.size)));
                const resp = await clobClient.createAndPostOrder({
                    tokenID: trade.asset,
                    price: useTradePrice ? (orderPrice || parseFloat(String(bestBid.price))) : parseFloat(String(bestBid.price)),
                    side: Side.SELL,
                    size: sellSize,
                }, { tickSize: "0.01" });
                if (resp.success) {
                    ...
                    await recordStatus(trade._id, followerId, 'SUCESSO', `Vendido tokens`, {
                        myEntryAmount: sellSize * parseFloat(String(bestBid.price)),
                        myEntryPrice: parseFloat(String(bestBid.price)),
                        myExecutedAt: new Date(),
                    });
                }
                else {
