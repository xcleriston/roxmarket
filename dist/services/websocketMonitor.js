...
const subscribeToTrades = (ws, traderAddress) => {
...
const handleWebSocketMessage = async (data, traderAddress) => {
    try {
        ...
        if (exists) {
            ...
            if (ageMinutes < 15) {
                processDetectedTrade(exists, traderAddress).catch(e => Logger.error(`Retry execution failed: ${e.message}`));
            }
            return;
        }
        ...
        processDetectedTrade(newTrade.toObject(), traderAddress).catch(e => Logger.error(`Direct execution failed: ${e.message}`));
    }
    catch (error) {
        Logger.error(`Error processing WebSocket message: ${error.message}`);
    }
};
const createWebSocketConnection = (traderAddress) => {
...
const pollTradesForTrader = async (traderAddress) => {
    try {
        const UserActivity = getUserActivityModel(traderAddress);
