...
Clique em "Gerar Carteira" para continuar:`;
    ...
    async handleConnectAccount(chatId) {
...
• Monitore suas operações regularmente`;
    ...
    async processPrivateKey(chatId, privateKey) {
        const user = await User.findOne({ chatId });
        if (!user)
            return;
        ...
        try {
            ...
            await this.sendMessage(chatId, `✅ *Conta Conectada com Sucesso!*\n\n📍 *Endereço:* \`${walletDerived.address}\`\n\nAgora vamos configurar quem você deseja copiar.`);
            // Directly jump to trader selection
            await this.handleDepositConfirmed(chatId);
        }
        catch (error) {
            await this.sendMessage(chatId, '❌ *Erro ao importar chave.* Verifique se a chave é válida e tente novamente.');
        }
    }
    async processTraderAddress(chatId, address) {
