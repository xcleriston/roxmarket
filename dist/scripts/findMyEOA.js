...
async function analyzeWallets() {
    ...
    try {
        const proxyPositions = await fetchData(`https://data-api.polymarket.com/positions?user=${PROXY_WALLET}`);
    ...
    try {
        const activities = await fetchData(`https://data-api.polymarket.com/activity?user=${PROXY_WALLET}&type=TRADE`);
