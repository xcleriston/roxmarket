...
const EOA_ADDRESS = ENV.PROXY_WALLET;
...
async function transferPositions() {
    ...
    const positions = await fetchData(`https://data-api.polymarket.com/positions?user=${EOA_ADDRESS}`);
    ...
    for (let i = 0; i < positions.length; i++) {
        ...
        try {
            ...
            const transferTx = await ctfContract.safeTransferFrom(EOA_ADDRESS, GNOSIS_SAFE_ADDRESS, pos.asset, balance, '0x', // empty data
            {
                gasPrice: gasPriceWithBuffer,
                gasLimit: 200000,
            });
    ...
    const eoaPositionsAfter = await fetchData(`https://data-api.polymarket.com/positions?user=${EOA_ADDRESS}`);
    const gnosisPositionsAfter = await fetchData(`https://data-api.polymarket.com/positions?user=${GNOSIS_SAFE_ADDRESS}`);
