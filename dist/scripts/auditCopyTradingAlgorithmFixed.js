...
async function simulateTrader(traderAddress, startingCapital) {
    ...
    try {
        ...
        return {
            ...
            winRate,
            avgTradeSize,
            openPositions: Array.from(positions.values()).filter((p) => !p.closed && p.sharesHeld > 0).length,
...
async function simulateCombinedBot(traderResults) {
    ...
    for (const result of validResults) {
        ...
        const winningPositions = Array.from(result.positions.values()).filter((p) => p.closed && p.pnl > 0).length;
...
function printAuditReport(report) {
    ...
    console.log(`  Copy Percentage: ${colors.yellow(report.config.copyPercentage + '%')} of trader's order size`);
    ...
    const deviationColor = Math.abs(analysis.roiDeviation) < 1
        ? colors.green
...
async function main() {
    ...
    console.log(`  Copy Percentage: ${colors.yellow(COPY_PERCENTAGE + '%')} of trader's order size`);
    ...
    try {
        ...
        const bestTrader = validResults.reduce((best, current) => (current.roi > best.roi ? current : best), validResults[0]);
        const worstTrader = validResults.reduce((worst, current) => (current.roi < worst.roi ? current : worst), validResults[0]);
