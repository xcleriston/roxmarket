 ...
 */
function calculateEquityCurve(trades, positions) {
    const equityPoints = [];
 ...
 */
function calculateSharpeRatio(equityPoints, riskFreeRate = 0) {
 ...
 */
function calculateVolatility(equityPoints) {
    if (equityPoints.length < 2) {
        return 0;
    }
    const returns = [];
 ...
 */
function calculateProfitFactor(trades, positions) {
    let grossProfit = 0;
    let grossLoss = 0;
 ...
 */
function calculateRiskScore(mdd, sharpeRatio, volatility, winRate) {
...
function printResults(results) {
    ...
    for (const trader of filtered.slice(0, 20)) {
        const statusColor = trader.status === 'excellent'
            ? colors.green
            : trader.status === 'good'
                ? colors.cyan
